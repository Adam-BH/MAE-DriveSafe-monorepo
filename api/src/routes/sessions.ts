...
router.get('/drivers/:id/sessions', authenticate, validateQuery(sessionsQuerySchema), async (req, res) => {
  const { id } = req.params;
  const { from, to, page, limit } = req.query as z.infer<typeof sessionsQuerySchema>;
